...
impl EthernetMac {
    ...
    #[inline(always)]
    pub const fn macsr(self) -> crate::common::Reg<regs::Macsr, crate::common::R> {
    ...
    #[inline(always)]
    pub const fn mmcrir(self) -> crate::common::Reg<regs::Mmcrir, crate::common::R> {
...
impl EthernetPtp {
    ...
    #[inline(always)]
    pub const fn ptpppscr(self) -> crate::common::Reg<regs::Ptpppscr, crate::common::RW> {
        unsafe { crate::common::Reg::from_ptr(self.ptr.wrapping_add(0x2cusize) as _) }
    }
}
pub mod regs {
    ...
    impl defmt::Format for Dmabmr {
        fn format(&self, f: defmt::Formatter) {
            defmt::write!(
                f,
                "Dmabmr {{ sr: {=bool:?}, da: {:?}, dsl: {=u8:?}, pbl: {:?}, rdp: {:?}, aab: {=bool:?} }}",
                self.sr(),
                self.da(),
                self.dsl(),
                self.pbl(),
    ...
    impl defmt::Format for Maccr {
        fn format(&self, f: defmt::Formatter) {
            defmt::write!(
                f,
                "Maccr {{ re: {=bool:?}, te: {=bool:?}, dc: {=bool:?}, bl: {:?}, apcs: {:?}, ipco: {:?}, dm: {:?}, lm: {:?}, rod: {:?}, fes: {:?}, csd: {:?}, ifg: {:?}, jd: {:?} }}",
                self.re(),
                self.te(),
                self.dc(),
                self.bl(),
    ...
    impl defmt::Format for Macffr {
        fn format(&self, f: defmt::Formatter) {
            defmt::write!(
                f,
                "Macffr {{ pm: {=bool:?}, hu: {:?}, hm: {:?}, daif: {:?}, pam: {=bool:?}, bfd: {:?}, saf: {=bool:?}, hpf: {:?}, ra: {=bool:?} }}",
                self.pm(),
                self.hu(),
                self.hm(),
                self.daif(),
    ...
    impl core::fmt::Debug for Macimr {
        fn fmt(&self, f: &mut core::fmt::Formatter) -> core::fmt::Result {
            f.debug_struct("Macimr").finish()
        }
    }
    #[cfg(feature = "defmt")]
    impl defmt::Format for Macimr {
        fn format(&self, f: defmt::Formatter) {
            defmt::write!(f, "Macimr {{ }}",)
        }
    }
    #[doc = "Ethernet MAC MII address register"]
    #[repr(transparent)]
    ...
    impl core::fmt::Debug for Mmcrimr {
        fn fmt(&self, f: &mut core::fmt::Formatter) -> core::fmt::Result {
            f.debug_struct("Mmcrimr").finish()
        }
    }
    #[cfg(feature = "defmt")]
    impl defmt::Format for Mmcrimr {
        fn format(&self, f: defmt::Formatter) {
            defmt::write!(f, "Mmcrimr {{ }}",)
        }
    }
    #[doc = "Ethernet MMC receive interrupt register"]
    #[repr(transparent)]
    ...
    impl core::fmt::Debug for Mmctimr {
        fn fmt(&self, f: &mut core::fmt::Formatter) -> core::fmt::Result {
            f.debug_struct("Mmctimr").finish()
        }
    }
    #[cfg(feature = "defmt")]
    impl defmt::Format for Mmctimr {
        fn format(&self, f: defmt::Formatter) {
            defmt::write!(f, "Mmctimr {{ }}",)
        }
    }
    #[doc = "Ethernet MMC transmit interrupt register"]
    #[repr(transparent)]
...
pub mod vals {
    /// MDC 软件请求；名称沿用 Embassy，合法 HCLK 范围仍以 GD 手册为准。
    #[derive(Copy, Clone, Debug, Eq, PartialEq)]
    pub struct Cr(Option<u8>);
    #[allow(non_upper_case_globals)]
    impl Cr {
        pub const Cr60100: Self = Self(Some(0)); // HCLK / 42
        pub const Cr100150: Self = Self(Some(1)); // HCLK / 62，GD 上限为 120MHz
        pub const Cr2035: Self = Self(Some(2)); // HCLK / 16
        pub const Cr3560: Self = Self(Some(3)); // HCLK / 26，不沿用上游描述中的 /16 笔误
        pub const Cr150168: Self = Self(None); // GD 没有 /102 编码
        pub const fn from_bits(value: u8) -> Self {
            if value < 4 { Self(Some(value)) } else { Self(None) }
        }
        pub const fn to_bits(self) -> u8 {
            match self.0 {
                Some(value) => value,
                None => panic!("GD32F30x 不支持此 MDC 分频请求，未写入寄存器"),
            }
        }
    }
...
impl regs::Macmiiar {
    pub const fn cr(&self) -> vals::Cr { vals::Cr::from_bits(((self.0 >> 2) & 7) as u8) }
    pub const fn set_cr(&mut self, request: vals::Cr) {
        // 在修改寄存器值之前验证软件请求；保留 PHY 地址、忙状态等其他位。
        let value = request.to_bits();
        self.0 = (self.0 & !0x1c) | ((value as u32) << 2);
    }
}
