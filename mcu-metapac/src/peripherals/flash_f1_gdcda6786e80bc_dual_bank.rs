#![allow(clippy::missing_safety_doc)]
#![allow(clippy::identity_op)]
#![allow(clippy::unnecessary_cast)]
#![allow(clippy::erasing_op)]

#[derive(Copy, Clone, Eq, PartialEq)]
pub struct Flash {
    ptr: *mut u8,
}
unsafe impl Send for Flash {}
unsafe impl Sync for Flash {}
impl Flash {
    #[inline(always)]
    pub const unsafe fn from_ptr(ptr: *mut ()) -> Self {
        Self { ptr: ptr as _ }
    }
    #[inline(always)]
    pub const fn as_ptr(&self) -> *mut () {
        self.ptr as _
    }
    #[doc = "Flash access control register"]
    #[inline(always)]
    pub const fn acr(self) -> AccessControl { AccessControl(self) }
    #[doc = "Flash key register"]
    #[inline(always)]
    pub const fn keyr(self) -> crate::common::Reg<u32, crate::common::W> {
        unsafe { crate::common::Reg::from_ptr(self.ptr.wrapping_add(0x04usize) as _) }
    }
    #[doc = "Flash option key register"]
    #[inline(always)]
    pub const fn optkeyr(self) -> crate::common::Reg<u32, crate::common::W> {
        unsafe { crate::common::Reg::from_ptr(self.ptr.wrapping_add(0x08usize) as _) }
    }
    #[doc = "Status register"]
    #[inline(always)]
    pub const fn sr(self) -> crate::common::Reg<regs::Sr, crate::common::RW> {
        unsafe { crate::common::Reg::from_ptr(self.ptr.wrapping_add(0x0cusize) as _) }
    }
    #[doc = "Control register"]
    #[inline(always)]
    pub const fn cr(self) -> crate::common::Reg<regs::Cr, crate::common::RW> {
        unsafe { crate::common::Reg::from_ptr(self.ptr.wrapping_add(0x10usize) as _) }
    }
    #[doc = "Flash address register"]
    #[inline(always)]
    pub const fn ar(self) -> crate::common::Reg<regs::Ar, crate::common::W> {
        unsafe { crate::common::Reg::from_ptr(self.ptr.wrapping_add(0x14usize) as _) }
    }
    #[doc = "Option byte register"]
    #[inline(always)]
    pub const fn obr(self) -> crate::common::Reg<regs::Obr, crate::common::R> {
        unsafe { crate::common::Reg::from_ptr(self.ptr.wrapping_add(0x1cusize) as _) }
    }
    #[doc = "Write protection register"]
    #[inline(always)]
    pub const fn wrpr(self) -> crate::common::Reg<regs::Wrpr, crate::common::R> {
        unsafe { crate::common::Reg::from_ptr(self.ptr.wrapping_add(0x20usize) as _) }
    }
}
pub mod regs {
    #[doc = "Bank 2 control register"]
    #[repr(transparent)]
    #[derive(Copy, Clone, Eq, PartialEq)]
    pub struct Cr2(pub u32);
    impl Cr2 {
        #[doc = "Programming"]
        #[must_use]
        #[inline(always)]
        pub const fn pg(&self) -> bool {
            let val = (self.0 >> 0usize) & 0x01;
            val != 0
        }
        #[doc = "Programming"]
        #[inline(always)]
        pub const fn set_pg(&mut self, val: bool) {
            self.0 = (self.0 & !(0x01 << 0usize)) | (((val as u32) & 0x01) << 0usize);
        }
        #[doc = "Page erase"]
        #[must_use]
        #[inline(always)]
        pub const fn per(&self) -> bool {
            let val = (self.0 >> 1usize) & 0x01;
            val != 0
        }
        #[doc = "Page erase"]
        #[inline(always)]
        pub const fn set_per(&mut self, val: bool) {
            self.0 = (self.0 & !(0x01 << 1usize)) | (((val as u32) & 0x01) << 1usize);
        }
        #[doc = "Bank 2 mass erase"]
        #[must_use]
        #[inline(always)]
        pub const fn mer(&self) -> bool {
            let val = (self.0 >> 2usize) & 0x01;
            val != 0
        }
        #[doc = "Bank 2 mass erase"]
        #[inline(always)]
        pub const fn set_mer(&mut self, val: bool) {
            self.0 = (self.0 & !(0x01 << 2usize)) | (((val as u32) & 0x01) << 2usize);
        }
        #[doc = "Start erase"]
        #[must_use]
        #[inline(always)]
        pub const fn strt(&self) -> bool {
            let val = (self.0 >> 6usize) & 0x01;
            val != 0
        }
        #[doc = "Start erase"]
        #[inline(always)]
        pub const fn set_strt(&mut self, val: bool) {
            self.0 = (self.0 & !(0x01 << 6usize)) | (((val as u32) & 0x01) << 6usize);
        }
        #[doc = "Bank 2 lock"]
        #[must_use]
        #[inline(always)]
        pub const fn lock(&self) -> bool {
            let val = (self.0 >> 7usize) & 0x01;
            val != 0
        }
        #[doc = "Bank 2 lock"]
        #[inline(always)]
        pub const fn set_lock(&mut self, val: bool) {
            self.0 = (self.0 & !(0x01 << 7usize)) | (((val as u32) & 0x01) << 7usize);
        }
        #[doc = "Error interrupt enable"]
        #[must_use]
        #[inline(always)]
        pub const fn errie(&self) -> bool {
            let val = (self.0 >> 10usize) & 0x01;
            val != 0
        }
        #[doc = "Error interrupt enable"]
        #[inline(always)]
        pub const fn set_errie(&mut self, val: bool) {
            self.0 = (self.0 & !(0x01 << 10usize)) | (((val as u32) & 0x01) << 10usize);
        }
        #[doc = "End of operation interrupt enable"]
        #[must_use]
        #[inline(always)]
        pub const fn eopie(&self) -> bool {
            let val = (self.0 >> 12usize) & 0x01;
            val != 0
        }
        #[doc = "End of operation interrupt enable"]
        #[inline(always)]
        pub const fn set_eopie(&mut self, val: bool) {
            self.0 = (self.0 & !(0x01 << 12usize)) | (((val as u32) & 0x01) << 12usize);
        }
    }
    impl Default for Cr2 {
        #[inline(always)]
        fn default() -> Cr2 {
            Cr2(0)
        }
    }
    impl core::fmt::Debug for Cr2 {
        fn fmt(&self, f: &mut core::fmt::Formatter) -> core::fmt::Result {
            f.debug_struct("Cr2")
                .field("pg", &self.pg())
                .field("per", &self.per())
                .field("mer", &self.mer())
                .field("strt", &self.strt())
                .field("lock", &self.lock())
                .field("errie", &self.errie())
                .field("eopie", &self.eopie())
                .finish()
        }
    }
    #[cfg(feature = "defmt")]
    impl defmt::Format for Cr2 {
        fn format(&self, f: defmt::Formatter) {
            defmt::write!(
                f,
                "Cr2 {{ pg: {=bool:?}, per: {=bool:?}, mer: {=bool:?}, strt: {=bool:?}, lock: {=bool:?}, errie: {=bool:?}, eopie: {=bool:?} }}",
                self.pg(),
                self.per(),
                self.mer(),
                self.strt(),
                self.lock(),
                self.errie(),
                self.eopie()
            )
        }
    }

    #[doc = "Flash access control register"]
    #[repr(transparent)]
    #[derive(Copy, Clone, Eq, PartialEq)]
    pub struct Acr(pub u32);
    impl Acr {
        #[doc = "Latency"]
        #[must_use]
        #[inline(always)]
        pub const fn latency(&self) -> super::vals::Latency {
            let val = (self.0 >> 0usize) & 0x07;
            super::vals::Latency::from_bits(val as u8)
        }
        #[doc = "Latency"]
        #[inline(always)]
        pub const fn set_latency(&mut self, val: super::vals::Latency) {
            self.0 = (self.0 & !(0x07 << 0usize)) | (((val.to_bits() as u32) & 0x07) << 0usize);
        }
    }
    impl Default for Acr {
        #[inline(always)]
        fn default() -> Acr {
            Acr(0)
        }
    }
    impl core::fmt::Debug for Acr {
        fn fmt(&self, f: &mut core::fmt::Formatter) -> core::fmt::Result {
            f.debug_struct("Acr").field("latency", &self.latency()).finish()
        }
    }
    #[cfg(feature = "defmt")]
    impl defmt::Format for Acr {
        fn format(&self, f: defmt::Formatter) {
            defmt::write!(f, "Acr {{ latency: {:?} }}", self.latency())
        }
    }
    #[doc = "Flash address register"]
    #[repr(transparent)]
    #[derive(Copy, Clone, Eq, PartialEq)]
    pub struct Ar(pub u32);
    impl Ar {
        #[doc = "Flash Address"]
        #[must_use]
        #[inline(always)]
        pub const fn far(&self) -> u32 {
            let val = (self.0 >> 0usize) & 0xffff_ffff;
            val as u32
        }
        #[doc = "Flash Address"]
        #[inline(always)]
        pub const fn set_far(&mut self, val: u32) {
            self.0 = (self.0 & !(0xffff_ffff << 0usize)) | (((val as u32) & 0xffff_ffff) << 0usize);
        }
    }
    impl Default for Ar {
        #[inline(always)]
        fn default() -> Ar {
            Ar(0)
        }
    }
    impl core::fmt::Debug for Ar {
        fn fmt(&self, f: &mut core::fmt::Formatter) -> core::fmt::Result {
            f.debug_struct("Ar").field("far", &self.far()).finish()
        }
    }
    #[cfg(feature = "defmt")]
    impl defmt::Format for Ar {
        fn format(&self, f: defmt::Formatter) {
            defmt::write!(f, "Ar {{ far: {=u32:?} }}", self.far())
        }
    }
    #[doc = "Control register"]
    #[repr(transparent)]
    #[derive(Copy, Clone, Eq, PartialEq)]
    pub struct Cr(pub u32);
    impl Cr {
        #[doc = "Programming"]
        #[must_use]
        #[inline(always)]
        pub const fn pg(&self) -> bool {
            let val = (self.0 >> 0usize) & 0x01;
            val != 0
        }
        #[doc = "Programming"]
        #[inline(always)]
        pub const fn set_pg(&mut self, val: bool) {
            self.0 = (self.0 & !(0x01 << 0usize)) | (((val as u32) & 0x01) << 0usize);
        }
        #[doc = "Page Erase"]
        #[must_use]
        #[inline(always)]
        pub const fn per(&self) -> bool {
            let val = (self.0 >> 1usize) & 0x01;
            val != 0
        }
        #[doc = "Page Erase"]
        #[inline(always)]
        pub const fn set_per(&mut self, val: bool) {
            self.0 = (self.0 & !(0x01 << 1usize)) | (((val as u32) & 0x01) << 1usize);
        }
        #[doc = "Mass Erase"]
        #[must_use]
        #[inline(always)]
        pub const fn mer(&self) -> bool {
            let val = (self.0 >> 2usize) & 0x01;
            val != 0
        }
        #[doc = "Mass Erase"]
        #[inline(always)]
        pub const fn set_mer(&mut self, val: bool) {
            self.0 = (self.0 & !(0x01 << 2usize)) | (((val as u32) & 0x01) << 2usize);
        }
        #[doc = "Option byte programming"]
        #[must_use]
        #[inline(always)]
        pub const fn optpg(&self) -> bool {
            let val = (self.0 >> 4usize) & 0x01;
            val != 0
        }
        #[doc = "Option byte programming"]
        #[inline(always)]
        pub const fn set_optpg(&mut self, val: bool) {
            self.0 = (self.0 & !(0x01 << 4usize)) | (((val as u32) & 0x01) << 4usize);
        }
        #[doc = "Option byte erase"]
        #[must_use]
        #[inline(always)]
        pub const fn opter(&self) -> bool {
            let val = (self.0 >> 5usize) & 0x01;
            val != 0
        }
        #[doc = "Option byte erase"]
        #[inline(always)]
        pub const fn set_opter(&mut self, val: bool) {
            self.0 = (self.0 & !(0x01 << 5usize)) | (((val as u32) & 0x01) << 5usize);
        }
        #[doc = "Start"]
        #[must_use]
        #[inline(always)]
        pub const fn strt(&self) -> bool {
            let val = (self.0 >> 6usize) & 0x01;
            val != 0
        }
        #[doc = "Start"]
        #[inline(always)]
        pub const fn set_strt(&mut self, val: bool) {
            self.0 = (self.0 & !(0x01 << 6usize)) | (((val as u32) & 0x01) << 6usize);
        }
        #[doc = "Lock"]
        #[must_use]
        #[inline(always)]
        pub const fn lock(&self) -> bool {
            let val = (self.0 >> 7usize) & 0x01;
            val != 0
        }
        #[doc = "Lock"]
        #[inline(always)]
        pub const fn set_lock(&mut self, val: bool) {
            self.0 = (self.0 & !(0x01 << 7usize)) | (((val as u32) & 0x01) << 7usize);
        }
        #[doc = "Option bytes write enable"]
        #[must_use]
        #[inline(always)]
        pub const fn optwre(&self) -> bool {
            let val = (self.0 >> 9usize) & 0x01;
            val != 0
        }
        #[doc = "Option bytes write enable"]
        #[inline(always)]
        pub const fn set_optwre(&mut self, val: bool) {
            self.0 = (self.0 & !(0x01 << 9usize)) | (((val as u32) & 0x01) << 9usize);
        }
        #[doc = "Error interrupt enable"]
        #[must_use]
        #[inline(always)]
        pub const fn errie(&self) -> bool {
            let val = (self.0 >> 10usize) & 0x01;
            val != 0
        }
        #[doc = "Error interrupt enable"]
        #[inline(always)]
        pub const fn set_errie(&mut self, val: bool) {
            self.0 = (self.0 & !(0x01 << 10usize)) | (((val as u32) & 0x01) << 10usize);
        }
        #[doc = "End of operation interrupt enable"]
        #[must_use]
        #[inline(always)]
        pub const fn eopie(&self) -> bool {
            let val = (self.0 >> 12usize) & 0x01;
            val != 0
        }
        #[doc = "End of operation interrupt enable"]
        #[inline(always)]
        pub const fn set_eopie(&mut self, val: bool) {
            self.0 = (self.0 & !(0x01 << 12usize)) | (((val as u32) & 0x01) << 12usize);
        }
    }
    impl Default for Cr {
        #[inline(always)]
        fn default() -> Cr {
            Cr(0)
        }
    }
    impl core::fmt::Debug for Cr {
        fn fmt(&self, f: &mut core::fmt::Formatter) -> core::fmt::Result {
            f.debug_struct("Cr")
                .field("pg", &self.pg())
                .field("per", &self.per())
                .field("mer", &self.mer())
                .field("optpg", &self.optpg())
                .field("opter", &self.opter())
                .field("strt", &self.strt())
                .field("lock", &self.lock())
                .field("optwre", &self.optwre())
                .field("errie", &self.errie())
                .field("eopie", &self.eopie())
                .finish()
        }
    }
    #[cfg(feature = "defmt")]
    impl defmt::Format for Cr {
        fn format(&self, f: defmt::Formatter) {
            defmt::write!(
                f,
                "Cr {{ pg: {=bool:?}, per: {=bool:?}, mer: {=bool:?}, optpg: {=bool:?}, opter: {=bool:?}, strt: {=bool:?}, lock: {=bool:?}, optwre: {=bool:?}, errie: {=bool:?}, eopie: {=bool:?} }}",
                self.pg(),
                self.per(),
                self.mer(),
                self.optpg(),
                self.opter(),
                self.strt(),
                self.lock(),
                self.optwre(),
                self.errie(),
                self.eopie()
            )
        }
    }
    #[doc = "Option byte register"]
    #[repr(transparent)]
    #[derive(Copy, Clone, Eq, PartialEq)]
    pub struct Obr(pub u32);
    impl Obr {
        #[doc = "Option byte error"]
        #[must_use]
        #[inline(always)]
        pub const fn opterr(&self) -> bool {
            let val = (self.0 >> 0usize) & 0x01;
            val != 0
        }
        #[doc = "Option byte error"]
        #[inline(always)]
        pub const fn set_opterr(&mut self, val: bool) {
            self.0 = (self.0 & !(0x01 << 0usize)) | (((val as u32) & 0x01) << 0usize);
        }
        #[doc = "Read protection"]
        #[must_use]
        #[inline(always)]
        pub const fn rdprt(&self) -> bool {
            let val = (self.0 >> 1usize) & 0x01;
            val != 0
        }
        #[doc = "Read protection"]
        #[inline(always)]
        pub const fn set_rdprt(&mut self, val: bool) {
            self.0 = (self.0 & !(0x01 << 1usize)) | (((val as u32) & 0x01) << 1usize);
        }
        #[doc = "WDG_SW"]
        #[must_use]
        #[inline(always)]
        pub const fn wdg_sw(&self) -> bool {
            let val = (self.0 >> 2usize) & 0x01;
            val != 0
        }
        #[doc = "WDG_SW"]
        #[inline(always)]
        pub const fn set_wdg_sw(&mut self, val: bool) {
            self.0 = (self.0 & !(0x01 << 2usize)) | (((val as u32) & 0x01) << 2usize);
        }
        #[doc = "nRST_STOP"]
        #[must_use]
        #[inline(always)]
        pub const fn n_rst_stop(&self) -> bool {
            let val = (self.0 >> 3usize) & 0x01;
            val != 0
        }
        #[doc = "nRST_STOP"]
        #[inline(always)]
        pub const fn set_n_rst_stop(&mut self, val: bool) {
            self.0 = (self.0 & !(0x01 << 3usize)) | (((val as u32) & 0x01) << 3usize);
        }
        #[doc = "nRST_STDBY"]
        #[must_use]
        #[inline(always)]
        pub const fn n_rst_stdby(&self) -> bool {
            let val = (self.0 >> 4usize) & 0x01;
            val != 0
        }
        #[doc = "nRST_STDBY"]
        #[inline(always)]
        pub const fn set_n_rst_stdby(&mut self, val: bool) {
            self.0 = (self.0 & !(0x01 << 4usize)) | (((val as u32) & 0x01) << 4usize);
        }
        #[doc = "Data0"]
        #[must_use]
        #[inline(always)]
        pub const fn data0(&self) -> u8 {
            let val = (self.0 >> 10usize) & 0xff;
            val as u8
        }
        #[doc = "Data0"]
        #[inline(always)]
        pub const fn set_data0(&mut self, val: u8) {
            self.0 = (self.0 & !(0xff << 10usize)) | (((val as u32) & 0xff) << 10usize);
        }
        #[doc = "Data1"]
        #[must_use]
        #[inline(always)]
        pub const fn data1(&self) -> u8 {
            let val = (self.0 >> 18usize) & 0xff;
            val as u8
        }
        #[doc = "Data1"]
        #[inline(always)]
        pub const fn set_data1(&mut self, val: u8) {
            self.0 = (self.0 & !(0xff << 18usize)) | (((val as u32) & 0xff) << 18usize);
        }
    }
    impl Default for Obr {
        #[inline(always)]
        fn default() -> Obr {
            Obr(0)
        }
    }
    impl core::fmt::Debug for Obr {
        fn fmt(&self, f: &mut core::fmt::Formatter) -> core::fmt::Result {
            f.debug_struct("Obr")
                .field("opterr", &self.opterr())
                .field("rdprt", &self.rdprt())
                .field("wdg_sw", &self.wdg_sw())
                .field("n_rst_stop", &self.n_rst_stop())
                .field("n_rst_stdby", &self.n_rst_stdby())
                .field("data0", &self.data0())
                .field("data1", &self.data1())
                .finish()
        }
    }
    #[cfg(feature = "defmt")]
    impl defmt::Format for Obr {
        fn format(&self, f: defmt::Formatter) {
            defmt::write!(
                f,
                "Obr {{ opterr: {=bool:?}, rdprt: {=bool:?}, wdg_sw: {=bool:?}, n_rst_stop: {=bool:?}, n_rst_stdby: {=bool:?}, data0: {=u8:?}, data1: {=u8:?} }}",
                self.opterr(),
                self.rdprt(),
                self.wdg_sw(),
                self.n_rst_stop(),
                self.n_rst_stdby(),
                self.data0(),
                self.data1()
            )
        }
    }
    #[doc = "Status register"]
    #[repr(transparent)]
    #[derive(Copy, Clone, Eq, PartialEq)]
    pub struct Sr(pub u32);
    impl Sr {
        #[doc = "Busy"]
        #[must_use]
        #[inline(always)]
        pub const fn bsy(&self) -> bool {
            let val = (self.0 >> 0usize) & 0x01;
            val != 0
        }
        #[doc = "Busy"]
        #[inline(always)]
        pub const fn set_bsy(&mut self, val: bool) {
            self.0 = (self.0 & !(0x01 << 0usize)) | (((val as u32) & 0x01) << 0usize);
        }
        #[doc = "Programming error"]
        #[must_use]
        #[inline(always)]
        pub const fn pgerr(&self) -> bool {
            let val = (self.0 >> 2usize) & 0x01;
            val != 0
        }
        #[doc = "Programming error"]
        #[inline(always)]
        pub const fn set_pgerr(&mut self, val: bool) {
            self.0 = (self.0 & !(0x01 << 2usize)) | (((val as u32) & 0x01) << 2usize);
        }
        #[doc = "Write protection error"]
        #[must_use]
        #[inline(always)]
        pub const fn wrprterr(&self) -> bool {
            let val = (self.0 >> 4usize) & 0x01;
            val != 0
        }
        #[doc = "Write protection error"]
        #[inline(always)]
        pub const fn set_wrprterr(&mut self, val: bool) {
            self.0 = (self.0 & !(0x01 << 4usize)) | (((val as u32) & 0x01) << 4usize);
        }
        #[doc = "End of operation"]
        #[must_use]
        #[inline(always)]
        pub const fn eop(&self) -> bool {
            let val = (self.0 >> 5usize) & 0x01;
            val != 0
        }
        #[doc = "End of operation"]
        #[inline(always)]
        pub const fn set_eop(&mut self, val: bool) {
            self.0 = (self.0 & !(0x01 << 5usize)) | (((val as u32) & 0x01) << 5usize);
        }
    }
    impl Default for Sr {
        #[inline(always)]
        fn default() -> Sr {
            Sr(0)
        }
    }
    impl core::fmt::Debug for Sr {
        fn fmt(&self, f: &mut core::fmt::Formatter) -> core::fmt::Result {
            f.debug_struct("Sr")
                .field("bsy", &self.bsy())
                .field("pgerr", &self.pgerr())
                .field("wrprterr", &self.wrprterr())
                .field("eop", &self.eop())
                .finish()
        }
    }
    #[cfg(feature = "defmt")]
    impl defmt::Format for Sr {
        fn format(&self, f: defmt::Formatter) {
            defmt::write!(
                f,
                "Sr {{ bsy: {=bool:?}, pgerr: {=bool:?}, wrprterr: {=bool:?}, eop: {=bool:?} }}",
                self.bsy(),
                self.pgerr(),
                self.wrprterr(),
                self.eop()
            )
        }
    }
    #[doc = "Write protection register"]
    #[repr(transparent)]
    #[derive(Copy, Clone, Eq, PartialEq)]
    pub struct Wrpr(pub u32);
    impl Wrpr {
        #[doc = "Write protect"]
        #[must_use]
        #[inline(always)]
        pub const fn wrp(&self) -> u32 {
            let val = (self.0 >> 0usize) & 0xffff_ffff;
            val as u32
        }
        #[doc = "Write protect"]
        #[inline(always)]
        pub const fn set_wrp(&mut self, val: u32) {
            self.0 = (self.0 & !(0xffff_ffff << 0usize)) | (((val as u32) & 0xffff_ffff) << 0usize);
        }
    }
    impl Default for Wrpr {
        #[inline(always)]
        fn default() -> Wrpr {
            Wrpr(0)
        }
    }
    impl core::fmt::Debug for Wrpr {
        fn fmt(&self, f: &mut core::fmt::Formatter) -> core::fmt::Result {
            f.debug_struct("Wrpr").field("wrp", &self.wrp()).finish()
        }
    }
    #[cfg(feature = "defmt")]
    impl defmt::Format for Wrpr {
        fn format(&self, f: defmt::Formatter) {
            defmt::write!(f, "Wrpr {{ wrp: {=u32:?} }}", self.wrp())
        }
    }
}
pub mod vals {
    #[repr(u8)]
    #[derive(Copy, Clone, Debug, Eq, PartialEq, Ord, PartialOrd)]
    #[cfg_attr(feature = "defmt", derive(defmt::Format))]
    pub enum Latency {
        #[doc = "Zero wait state, if 0 < SYSCLK≤ 24 MHz"]
        Ws0 = 0x0,
        #[doc = "One wait state, if 24 MHz < SYSCLK ≤ 48 MHz"]
        Ws1 = 0x01,
        #[doc = "Two wait states, if 48 MHz < SYSCLK ≤ 72 MHz"]
        Ws2 = 0x02,
        _RESERVED_3 = 0x03,
        _RESERVED_4 = 0x04,
        _RESERVED_5 = 0x05,
        _RESERVED_6 = 0x06,
        _RESERVED_7 = 0x07,
    }
    impl Latency {
        #[inline(always)]
        pub const fn from_bits(val: u8) -> Latency {
            unsafe { core::mem::transmute(val & 0x07) }
        }
        #[inline(always)]
        pub const fn to_bits(self) -> u8 {
            unsafe { core::mem::transmute(self) }
        }
    }
    impl From<u8> for Latency {
        #[inline(always)]
        fn from(val: u8) -> Latency {
            Latency::from_bits(val)
        }
    }
    impl From<Latency> for u8 {
        #[inline(always)]
        fn from(val: Latency) -> u8 {
            Latency::to_bits(val)
        }
    }
}
/// GD32F30x 的 Flash 访问策略，不是额外的硬件寄存器。
#[derive(Copy, Clone, Eq, PartialEq)]
pub struct AccessControl(Flash);

/// 软件请求与真实 WSCNT 字段分离；GD 没有 STM32 PRFTBE 位。
pub struct AccessConfig {
    latency: vals::Latency,
    commit: bool,
}

impl AccessConfig {
    pub fn latency(&self) -> vals::Latency {
        self.latency
    }
    pub fn set_latency(&mut self, value: vals::Latency) {
        assert!(value.to_bits() <= 2, "GD32F30x 只允许 0、1、2 个等待周期");
        self.latency = value;
        self.commit = true;
    }
    /// Embassy 的预取开启请求采用 GD 等待周期策略；不提供预取功能或性能保证。
    /// 禁用预取没有已证明的对应行为，必须在硬件写入前拒绝。
    pub fn set_prftbe(&mut self, enable: bool) {
        assert!(enable, "GD32F30x 不支持 STM32 预取禁用策略");
        self.commit = true;
    }
}

impl AccessControl {
    /// 返回真实 FMC_WS 内容，不合成 PRFTBE 字段。
    pub fn read(&self) -> regs::Acr {
        // 安全性：沿用 Flash::from_ptr 的有效外设映射前提，FMC_WS 要求 32 位访问。
        regs::Acr(unsafe { self.0.ptr.cast::<u32>().read_volatile() })
    }

    pub fn modify<R>(&self, f: impl FnOnce(&mut AccessConfig) -> R) -> R {
        #[cfg(target_arch = "arm")]
        {
            // 单核 GD32F30x：屏蔽可屏蔽中断，覆盖读配置、调用闭包及整个解锁事务。
            // NMI、HardFault 和 DMA 不得操作 FMC；与原生 PAC 相同，不提供跨主机互斥。
            cortex_m::interrupt::free(|_| {
                let mut config = AccessConfig {
                    latency: self.read().latency(),
                    commit: false,
                };
                let result = f(&mut config);
                if config.commit {
                    // 安全性：官方 FMC_SIZE 是此系列固定的只读 16 位容量信息，单位 KiB。
                    let size_kib = unsafe { (0x1fff_f7e0 as *const u16).read_volatile() };
                    assert!(size_kib != 0 && size_kib != u16::MAX, "Flash 容量信息无效");
                    let banks = if size_kib > 512 { 2 } else { 1 };
                    let ok = configure_wait_states(
                        |offset, value| {
                            // 安全性：偏移仅由下方已校验的 FMC 事务生成，均按 32 位对齐。
                            let ptr = unsafe { self.0.ptr.add(offset).cast::<u32>() };
                            match value {
                                Some(value) => {
                                    unsafe { ptr.write_volatile(value) };
                                    value
                                }
                                None => unsafe { ptr.read_volatile() },
                            }
                        },
                        banks,
                        u32::from(config.latency.to_bits()),
                    );
                    assert!(ok, "FMC 等待周期配置失败，禁止继续提升时钟");
                }
                result
            })
        }
        #[cfg(not(target_arch = "arm"))]
        {
            let _ = f;
            panic!("Flash 硬件策略仅用于 ARM GD32F30x");
        }
    }
}

/// 与实际 MMIO 共用的事务；测试注入寄存器读写以验证受保护寄存器行为。
/// 返回前总会尝试恢复本事务打开的锁，不等待可能永久忙碌的硬件。
fn configure_wait_states(
    mut io: impl FnMut(usize, Option<u32>) -> u32,
    banks: usize,
    count: u32,
) -> bool {
    if !(1..=2).contains(&banks) || count > 2 {
        return false;
    }
    let mut controls = [0u32; 2];
    let ws = io(0, None);
    let enable = io(0xfc, None);
    if ws & !7 != 0 || enable & !3 != 0 {
        return false;
    }
    for (bank, control) in controls.iter_mut().enumerate().take(banks) {
        let offset = bank * 0x40;
        *control = io(0x10 + offset, None);
        // 拒绝忙碌或仍配置为擦写模式的控制器，不能在重锁时重发命令。
        if io(0x0c + offset, None) & 1 != 0 || *control & 0x77 != 0 {
            return false;
        }
    }
    let mut ok = true;
    for (bank, control) in controls.iter().enumerate().take(banks) {
        let offset = bank * 0x40;
        if control & 0x80 != 0 {
            io(0x04 + offset, Some(0x4567_0123));
            io(0x04 + offset, Some(0xcdef_89ab));
        }
        if io(0x10 + offset, None) & 0x80 != 0 {
            ok = false;
            break;
        }
    }
    if ok {
        io(0, Some(count));
        io(0xfc, Some(enable | 1));
        ok = io(0, None) == count && io(0xfc, None) == enable | 1;
    }
    for (bank, control) in controls.iter().enumerate().take(banks) {
        let offset = 0x10 + bank * 0x40;
        if control & 0x80 != 0 {
            io(offset, Some(*control));
        }
        ok &= io(offset, None) == *control;
    }
    ok
}

/// 第二bank寄存器；仅有真实双bank的芯片引用本模块。
impl Flash {
    pub const fn keyr2(self) -> crate::common::Reg<u32, crate::common::W> {
        // 安全性：官方FMC_KEY1偏移0x44，32位只写。
        unsafe { crate::common::Reg::from_ptr(self.ptr.wrapping_add(0x44).cast()) }
    }
    pub const fn sr2(self) -> crate::common::Reg<regs::Sr, crate::common::RW> {
        // 安全性：官方FMC_STAT1偏移0x4c，与STAT0的已定义状态位相同。
        unsafe { crate::common::Reg::from_ptr(self.ptr.wrapping_add(0x4c).cast()) }
    }
    pub const fn cr2(self) -> crate::common::Reg<regs::Cr2, crate::common::RW> {
        // 安全性：官方FMC_CTL1偏移0x50；类型不提供bank0独有的选项字节位。
        unsafe { crate::common::Reg::from_ptr(self.ptr.wrapping_add(0x50).cast()) }
    }
    pub const fn ar2_raw(self) -> crate::common::Reg<regs::Ar, crate::common::W> {
        // 安全性：官方FMC_ADDR1偏移0x54，32位只写。
        unsafe { crate::common::Reg::from_ptr(self.ptr.wrapping_add(0x54).cast()) }
    }
    pub const fn ar2(self) -> Bank2Address { Bank2Address(self) }
}
/// 明确的页擦除地址事务，不是新增寄存器。
pub struct Bank2Address(Flash);
impl Bank2Address {
    pub fn write(&self, f: impl FnOnce(&mut regs::Ar)) {
        let mut value = regs::Ar::default();
        f(&mut value);
        self.0.ar2_raw().write_value(value);
        // 官方fmc_page_erase及手册2.3.4要求保护态同时提交ADDR0。
        if self.0.obr().read().0 & 2 != 0 { self.0.ar().write_value(value); }
    }
}
