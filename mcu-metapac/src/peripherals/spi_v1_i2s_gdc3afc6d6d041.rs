...
pub mod regs {
    ...
    impl defmt::Format for Cr2 {
        fn format(&self, f: defmt::Formatter) {
            defmt::write!(
                f,
                "Cr2 {{ rxdmaen: {=bool:?}, txdmaen: {=bool:?}, ssoe: {=bool:?}, errie: {=bool:?}, rxneie: {=bool:?}, txeie: {=bool:?} }}",
                self.rxdmaen(),
                self.txdmaen(),
                self.ssoe(),
                self.errie(),
    ...
    impl defmt::Format for Sr {
        fn format(&self, f: defmt::Formatter) {
            defmt::write!(
                f,
                "Sr {{ rxne: {=bool:?}, txe: {=bool:?}, chside: {:?}, udr: {=bool:?}, crcerr: {=bool:?}, modf: {=bool:?}, ovr: {=bool:?}, bsy: {=bool:?} }}",
                self.rxne(),
                self.txe(),
                self.chside(),
                self.udr(),
